...
class TileWorker(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            # filter the tile for background
            pixels = np.array(tile)
            mean = sum([pixels[:, :, i].mean() for i in range(3)])/3
            std = sum([pixels[:, :, i].std() for i in range(3)])
            if mean < 200 and std < 160:
...
class DeepZoomImageTiler(object):
    ...
    def __init__(self, dz, basename, format, associated, queue, slidepath):
        # use highest level magnification available
        self._dz = dz
        ...
        # this is where the magnification filtering happens
        # max mag
        #self.level = self._dz.level_count - 1
        # 5x mag w/ 40x objective
        # self.level = self._dz.level_count - 4
        MAGNIFICATION = 20

        objective = int(self._slide.properties[openslide.PROPERTY_NAME_OBJECTIVE_POWER])
        for i, j in enumerate(range(self._dz.level_count)):
            if objective == MAGNIFICATION:
                self.level = self._dz.level_count - j - 1
                break
    ...
    def _write_tiles(self):
        self._basename = '-'.join(self._basename.split('-')[:3])
        tiledir = '{0}'.format(self._basename)

        if not os.path.exists(tiledir):
            os.makedirs(tiledir)

        cols, rows = self._dz.level_tiles[self.level]
        for row in range(rows):
            for col in range(cols):
                tilename = os.path.join(tiledir, '%d_%d.%s' % (
                                col, row, self._format))
                if not os.path.exists(tilename):
                    self._queue.put((self._associated, self.level, (col, row),
                                tilename))
                self._tile_done()

    def _tile_done(self):
        self._processed += 1
        count, total = self._processed, self._dz.level_tiles[self.level][0]*self._dz.level_tiles[self.level][1]
        if count % 100 == 0 or count == total:
            print("Tiling %s: processed %d/%d tiles" % (
